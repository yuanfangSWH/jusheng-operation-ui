  ...
  <div class="app-container">
    <div class="mb20 flex transposition justify-end">
      ...
      >
        ...
        <el-option value="3" label="承运司机"></el-option>
      </el-select>
      ...
      <el-input class="search mr5" v-model="numberData" clearable></el-input>
    ...
    >
      ...
      <el-table-column prop="accountBank" label="提现账户" width="700px">
        <template slot-scope="scope">
          {{ scope.row.kaihuName }}（{{ scope.row.accountBank }}/{{
            scope.row.kaihuBank
          }}）
        </template>
      </el-table-column>
      <el-table-column prop="totalMoney" label="账户总额" width="150">
        <template slot-scope="scope"> {{ scope.row.totalMoney }}元 </template>
      </el-table-column>
      <el-table-column prop="usableMoney" label="可用余额" width="150">
        <template slot-scope="scope"> {{ scope.row.usableMoney }}元 </template>
      </el-table-column>
      <el-table-column prop="freezeMoney" label="冻结金额" width="150">
    ...
    >
      ...
      >
        ...
        <el-form-item label="银行卡号" prop="bankCardNumber">
          <el-input v-model="queryCard.bankCardNumber" clearable></el-input>
        </el-form-item>
        <el-form-item label="持卡人姓名" prop="cardholderName">
          <el-input v-model="queryCard.cardholderName" clearable></el-input>
        </el-form-item>
        <el-form-item label="持卡人身份证号" prop="cardholderIDNumber">
          <el-input v-model="queryCard.cardholderIDNumber" clearable></el-input>
        </el-form-item>
      </el-form>
    ...
    >
      <el-form
        ...
        label-width="100px"
      >
        <el-form-item label="扣款用户" prop="kaihuName">
          <template>
            <span>{{ queryToDown.kaihuName }}</span>
          </template>
        </el-form-item>
        <el-form-item label="用户卡号" prop="accountBank">
          <template>
            <span>{{ queryToDown.accountBank }}</span>
          </template>
        </el-form-item>
        <el-form-item label="可用余额" prop="usableMoney">
          <template>
            <span>{{ queryToDown.usableMoney }}元</span>
          </template>
        </el-form-item>
        <el-form-item label="扣款金额" prop="amountOfDeductions">
          <el-input
            v-model="queryToDown.amountOfDeductions"
            clearable
          ></el-input
          >（元）
        </el-form-item>
    ...
    >
      <el-form
        ...
        label-width="100px"
      >
        <el-form-item label="充值用户" prop="kaihuName">
          <template>
            <span>{{ queryRecharge.kaihuName }}</span>
          </template>
        </el-form-item>
        <el-form-item label="用户卡号" prop="accountBank">
          <template>
            <span>{{ queryRecharge.accountBank }}</span>
          </template>
        </el-form-item>
        <el-form-item label="可用余额" prop="usableMoney">
          <template>
            <span>{{ queryRecharge.usableMoney }}元</span>
          </template>
        </el-form-item>
        <el-form-item label="充值金额" prop="topUpAmount">
          <el-input v-model="queryRecharge.topUpAmount" clearable></el-input
          >（元）
        </el-form-item>
        <el-form-item label="备注" prop="remark">
          <el-input
...
<script>
import { getList, clientAccountTopUp, clientDeductions, queryBankCardInfo, updateDriverBankCard } from "@/api/finance/wallet";
...
export default {
  ...
  data () {
    return {
      ...
      rules3: {
        ...
        cardholderIDNumber: [
          { required: true, message: '持卡人身份证号不能为空' }
        ]
      },
      dialogVisible: false,
      ...
      conditions_data: "userName", //下拉框选中的值
      numberData: "", //输入框
  ...
  methods: {
    ...
    goInvoice (data) {
      ...
      this.$router.push({ path: '/finance/systemTransactionRecord', query: { userId: data.userId, type: data.type } });
    },
    ...
    getRecharge (row) {
      ...
      this.queryRecharge.kaihuName = row.kaihuName;
      this.queryRecharge.accountBank = row.accountBank
    ...
    getDeduction (row) {
      ...
      this.queryToDown.kaihuName = row.kaihuName;
      this.queryToDown.accountBank = row.accountBank;
    ...
    getBankCark (row) {
      ...
      queryBankCardInfo({ chauffeurId: row.userId }).then(res => {
        if (res.code == 200) {
          this.queryCard.cardholderName = res.data?.cardholderName
          this.queryCard.bankCardNumber = res.data?.bankCardNumber
          this.queryCard.cardholderIDNumber = res.data?.cardholderIDNumber
    ...
    cardBtn () {
      this.$refs.queryCard.validate((valid) => {
        if (valid) {
          updateDriverBankCard(this.queryCard).then(res => {
            if (res.code == 200) {
              this.$message({
                message: '操作成功',
                type: 'success',
