  ...
  <div class="app-container">
    <el-form ref="form" :model="form" :rules="rules" label-width="150px" inline>
      ...
      <el-form-item label="身份信息">
        <el-radio-group v-model="form.identity" @change="wenrou">
          ...
          <el-radio :label="0" disabled>个人/经纪人</el-radio>
        </el-radio-group>
      </el-form-item>
      <div v-show="form.identity == 0">
        ...
        <el-form-item label="姓名" prop="shipperFullName">
          ...
          <el-button
            type="primary"
            style="margin-left: 10px"
            @click="detection2('remind')"
            :disabled="form.shipperFullName == ''"
            >检测</el-button
          >
        </el-form-item>
    ...
    <div class="flex justify-center">
      <el-button
        ...
        >返回</el-button
      >
      <el-button type="primary" @click="premise()">保存提交</el-button>
    </div>
  </div>
</template>
...
import { saveShipperUser, queryAdminCompany, verifyUserName, verifyEnterpriseName } from "@/api/member/shipper.js";
  ...
  methods: {
    ...
    detection1 (key = "") {
      return new Promise((resolve, reject) => {
        if (this.form.userName != "") {
          verifyUserName({ userName: this.form.userName }).then((res) => {
            if (!res.data) {
              if (key == "remind") {
                this.$message({
                  message: "用户名可以使用",
        ...
        if (this.form.shipperFullName != "") {
          verifyEnterpriseName({ shipperFullName: this.form.shipperFullName }).then((res) => {
            if (!res.data) {
              ...
              if (key == "remind") {
                this.$message({
                  message: (this.form.identity == 1 ? "托运人全称" : "姓名") + "已存在，不可使用",
                  type: 'error',
                  duration: 3000
                });
              } else {
    ...
    premise () {
      this.$refs.form.validate((valid) => {
        if (valid) {
          const SWH1 = this.detection1(1)
          const SWH2 = this.detection2(1)
          Promise.all([SWH1, SWH2]).then(() => {
            this.saveShipperUser();
          }).catch((res) => {
            if (res == 1) {
              this.$message({
                message: "用户名已存在，不可使用，请重新填写后再提交",
                type: 'error',
                duration: 3000
              });
            } else if (res == 2) {
              this.$message({
                message: (this.form.identity == 1 ? "托运人全称" : "姓名") + "已存在，不可使用，请重新填写后再提交",
                type: 'error',
                duration: 3000
              });
            }
