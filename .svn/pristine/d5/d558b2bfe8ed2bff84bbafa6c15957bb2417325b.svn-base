<template>
  <div class="app-container">
    <el-form ref="form" :model="form" :rules="rules" label-width="150px">
      <div class="mb20">
        <el-divider content-position="left">
          <span class="weight">手机号信息</span>
        </el-divider>
      </div>
      <el-form-item label="手机号" prop="phone">
        <el-input
          class="search"
          v-model="form.phone"
          placeholder="请输入手机号"
          clearable
        ></el-input>
        <el-button type="primary" style="margin-left: 10px" @click="phoneQuery"
          >检测手机号是否被注册</el-button
        >
        <span
          style="margin-left: 20px; color: #67c23a"
          v-show="phoneProblem == 1"
          >恭喜，该手机号可以使用~</span
        ><span
          style="margin-left: 20px; color: #f56c6c"
          v-show="phoneProblem == 2"
          >该手机号已被注册，请换一个手机号再试试！</span
        >
      </el-form-item>
      <!-- <el-form-item label="司机密码" prop="password">
        <el-input class="search"
          v-model="form.password"
          placeholder="请输入司机密码"
          clearable
        ></el-input>
      </el-form-item> -->
      <div class="mb20">
        <el-divider content-position="left">
          <span class="weight">身份信息</span>
        </el-divider>
      </div>

      <el-row class="flex align-center">
        <el-col :span="5">
          <el-form-item label="身份证头像面" prop="idCardPerson">
            <drivingOCR
              ref="drivingOCR1"
              @drivingImg="drivingImg"
              @pictureDelete="pictureDelete"
              componentText="添加身份证头像面"
              fieldText="idCardPerson"
              OCRtext="IDCardOCRAndUpload"
            ></drivingOCR
          ></el-form-item>
        </el-col>
        <el-col :span="13">
          <el-form-item label="姓名" prop="name">
            <el-input
              class="search"
              v-model="form.name"
              placeholder="请输入姓名"
              clearable
            ></el-input>
          </el-form-item>
          <el-form-item label="身份证号" prop="idCardNumber">
            <el-input
              class="search"
              v-model="form.idCardNumber"
              placeholder="请输入身份证号"
              clearable
            ></el-input>
          </el-form-item>
          <el-form-item label="身份证有效期" prop="idCardLongEff">
            <el-select v-model="form.idCardLongEff" placeholder="身份证有效期">
              <el-option label="非长期有效" :value="1"></el-option>
              <el-option label="长期有效" :value="0"></el-option>
            </el-select>
          </el-form-item>
        </el-col>
      </el-row>

      <el-row class="flex align-center">
        <el-col :span="5">
          <el-form-item label="身份证国徽面" prop="idCardBadge">
            <drivingOCR
              ref="drivingOCR1"
              @drivingImg="drivingImg"
              @pictureDelete="pictureDelete"
              componentText="添加身份证国徽面"
              fieldText="idCardBadge"
              OCRtext="IDCardOCRAndUpload"
            ></drivingOCR>
          </el-form-item>
        </el-col>
        <el-col :span="13">
          <el-form-item label="身份证有效开始日期" prop="idCardPeriodStartTime">
            <el-date-picker
              v-model="form.idCardPeriodStartTime"
              format="yyyy 年 MM 月 dd 日"
              value-format="yyyy-MM-dd"
              type="date"
              placeholder="选择开始日期"
            >
            </el-date-picker>
          </el-form-item>
          <el-form-item label="身份证有效结束日期" prop="idCardPeriodEndTime">
            <el-date-picker
              :disabled="form.idCardLongEff == 0 ? true : false"
              v-model="form.idCardPeriodEndTime"
              format="yyyy 年 MM 月 dd 日"
              value-format="yyyy-MM-dd"
              type="date"
              placeholder="选择结束日期"
            >
            </el-date-picker>
          </el-form-item>
        </el-col>
      </el-row>

      <div class="mb20">
        <el-divider content-position="left">
          <span class="weight">驾驶证信息</span>
        </el-divider>
      </div>

      <el-row class="flex align-center">
        <el-col :span="5">
          <el-form-item label="驾驶证正本" prop="drivingPerson">
            <drivingOCR
              ref="drivingOCR1"
              @drivingImg="drivingImg"
              @pictureDelete="pictureDelete"
              componentText="添加驾驶证正本"
              fieldText="drivingPerson"
              OCRtext="DLCardOCRAndUpload"
            ></drivingOCR>
          </el-form-item>
        </el-col>
        <el-col :span="13">
          <el-form-item label="驾驶证号" prop="drivingNumber">
            <el-input
              class="search"
              v-model="form.drivingNumber"
              placeholder="请输入驾驶证号"
              clearable
            ></el-input>
          </el-form-item>
          <el-form-item label="准驾车型" prop="vehicleType">
            <el-input
              class="search"
              v-model="form.vehicleType"
              placeholder="请输入准驾车型"
              clearable
            ></el-input>
          </el-form-item>
          <el-form-item label="驾驶证签发机关" prop="licenseIssuingAuthority">
            <el-input
              class="search"
              v-model="form.licenseIssuingAuthority"
              placeholder="请输入驾驶证签发机关"
              clearable
            ></el-input>
          </el-form-item>
        </el-col>
      </el-row>

      <el-row class="flex align-center">
        <el-col :span="5">
          <el-form-item label="驾驶证副本" prop="drivingBadge">
            <drivingOCR
              ref="drivingOCR1"
              @drivingImg="drivingImg"
              @pictureDelete="pictureDelete"
              componentText="添加驾驶证副本"
              fieldText="drivingBadge"
              OCRtext="DLCardOCRAndUpload"
            ></drivingOCR>
          </el-form-item>
        </el-col>
        <el-col :span="13">
          <el-form-item label="驾驶证有效开始日期" prop="drivingStartTime">
            <el-date-picker
              v-model="form.drivingStartTime"
              format="yyyy 年 MM 月 dd 日"
              value-format="yyyy-MM-dd"
              type="date"
              placeholder="选择开始日期"
            >
            </el-date-picker>
          </el-form-item>
          <el-form-item label="驾驶证有效结束日期" prop="drivingEndTime">
            <el-date-picker
              v-model="form.drivingEndTime"
              format="yyyy 年 MM 月 dd 日"
              value-format="yyyy-MM-dd"
              type="date"
              placeholder="选择结束日期"
            >
            </el-date-picker>
          </el-form-item>
        </el-col>
      </el-row>

      <div class="mb20">
        <el-divider content-position="left">
          <span class="weight">从业资格证信息（非必填）</span>
        </el-divider>
      </div>

      <el-row class="flex align-center">
        <el-col :span="5">
          <el-form-item label="从业资格证图1" prop="cyCertificate">
            <uploadIMG
              ref="uploadIMG5"
              @pictureIMG="pictureIMG"
              @pictureDelete="pictureDelete"
              componentText="添加从业资格证图1"
              fieldText="cyCertificate"
            ></uploadIMG>
          </el-form-item>
        </el-col>
        <el-col :span="13">
          <el-form-item label="从业资格证号" prop="qualifiedNumber">
            <el-input
              class="search"
              v-model="form.qualifiedNumber"
              clearable
            ></el-input>
          </el-form-item>
        </el-col>
      </el-row>
      <el-row class="flex align-center">
        <el-col :span="5">
          <el-form-item label="从业资格证图2" prop="cyCertificateTwo">
            <uploadIMG
              ref="uploadIMG6"
              @pictureIMG="pictureIMG"
              @pictureDelete="pictureDelete"
              componentText="添加从业资格证图2"
              fieldText="cyCertificateTwo"
            ></uploadIMG>
          </el-form-item>
        </el-col>
        <el-col :span="13">
          <el-form-item
            label="资格证有效开始日期"
            prop="qualifiedPeriodStartTime"
          >
            <el-date-picker
              v-model="form.qualifiedPeriodStartTime"
              format="yyyy 年 MM 月 dd 日"
              value-format="yyyy-MM-dd"
              type="date"
              placeholder="选择开始日期"
            >
            </el-date-picker>
          </el-form-item>
          <el-form-item
            label="资格证有效结束日期"
            prop="qualifiedPeriodEndTime"
          >
            <el-date-picker
              v-model="form.qualifiedPeriodEndTime"
              format="yyyy 年 MM 月 dd 日"
              value-format="yyyy-MM-dd"
              type="date"
              placeholder="选择结束日期"
            >
            </el-date-picker> </el-form-item
        ></el-col>
      </el-row>
      <div class="mb20">
        <el-divider content-position="left">
          <span class="weight">其它信息</span>
        </el-divider>
      </div>
      <el-row class="flex align-center">
        <el-col :span="5">
          <el-form-item label="关联车辆" prop="vehicleNumber">
            <el-autocomplete
              v-model="cheState"
              :fetch-suggestions="querySearchAsync"
              placeholder="车牌号模糊搜索"
              @select="handleSelect"
            ></el-autocomplete>
          </el-form-item>
        </el-col>
      </el-row>
    </el-form>
    <div class="flex justify-center">
      <el-button
        @click="
          () => {
            this.$router.go(-1);
          }
        "
        >返回</el-button
      >
      <el-button type="primary" @click="AddData()">保存提交</el-button>
    </div>
  </div>
</template>

<script>
import drivingOCR from '@/components/drivingOCR/index'
import uploadIMG from '@/components/uploadIMG/index'
import { saveDriver, detectionPhone, queryVehicleNumber } from '@/api/member/driver'
import { phoneCheck } from "@/utils/check.js";
export default {
  components: {
    drivingOCR,
    uploadIMG
  },
  data () {
    return {
      form: {
        //手机号
        phone: "",
        //司机密码
        password: "",
        //身份证头像面
        idCardPerson: "",
        //姓名
        name: "",
        //身份证
        idCardNumber: "",
        //身份证有效期
        idCardLongEff: 1,
        //身份证国徽面
        idCardBadge: "",
        //身份证有效开始日期
        idCardPeriodStartTime: "",
        //身份证有效结束日期
        idCardPeriodEndTime: "",
        //驾驶证正本
        drivingPerson: "",
        //驾驶证号
        drivingNumber: "",
        //准驾车型
        vehicleType: "",
        //驾驶证签发机关
        licenseIssuingAuthority: "",
        //驾驶证副本
        drivingBadge: "",
        //驾驶证有效开始日期
        drivingStartTime: "",
        //驾驶证有效结束日期
        drivingEndTime: "",
        //从业资格证
        cyCertificate: "",
        cyCertificateTwo: "",
        //从业资格证号
        qualifiedNumber: "",
        //资格证有效开始日期
        qualifiedPeriodStartTime: "",
        //资格证有效结束日期
        qualifiedPeriodEndTime: "",
        //关联车辆
        vehicleNumber: ""
      },
      //编辑提交的车辆数据
      cheData: {},
      cheState: "",
      //检测结果
      phoneProblem: 0,
      rules: {
        phone: [
          { required: true, message: '手机号不能为空' }
        ],
        idCardPerson: [
          { required: true, message: '身份证头像面不能为空' }
        ],
        name: [
          { required: true, message: '姓名不能为空' }
        ],
        idCardNumber: [
          { required: true, message: '身份证不能为空' }
        ],
        idCardLongEff: [
          { required: true, message: '身份证有效期不能为空' }
        ],
        idCardBadge: [
          { required: true, message: '身份证国徽面不能为空' }
        ],
        idCardPeriodStartTime: [
          { required: true, message: '身份证有效开始日期不能为空' }
        ],
        idCardPeriodEndTime: [
          { required: true, message: '身份证有效结束日期不能为空' }
        ],
        drivingPerson: [
          { required: true, message: '驾驶证正本不能为空' }
        ],
        drivingNumber: [
          { required: true, message: '驾驶证号不能为空' }
        ],
        vehicleType: [
          { required: true, message: '准驾车型不能为空' }
        ],
        licenseIssuingAuthority: [
          { required: true, message: '驾驶证签发机关不能为空' }
        ],
        drivingBadge: [
          { required: true, message: '驾驶证副本不能为空' }
        ],
        drivingStartTime: [
          { required: true, message: '驾驶证有效开始日期不能为空' }
        ],
        drivingEndTime: [
          { required: true, message: '驾驶证有效结束日期不能为空' }
        ]
      }
    }
  },
  mounted () {

  },
  methods: {
    //检测手机号
    phoneQuery () {
      if (this.form.phone == "" || phoneCheck(this.form.phone)) {
        detectionPhone({ mobilePhone: this.form.phone }).then((res) => {
          if (res) {
            this.phoneProblem = 1;
          } else {
            this.phoneProblem = 2;
          }
        });
      }
    },
    //添加之前
    AddData () {
      if (this.phoneProblem == 0) {
        if (this.form.phone == "" || phoneCheck(this.form.phone)) {
          detectionPhone({ mobilePhone: this.form.phone }).then((res) => {
            if (res) {
              this.phoneProblem = 1;
              this.submitForm();
            } else {
              this.phoneProblem = 2;
            }
          });
        }
      } else {
        this.submitForm();
      }
    },
    //添加
    submitForm () {
      if (this.form.idCardLongEff == 0) {
        this.form.idCardPeriodEndTime = "null"
      }
      this.$refs.form.validate((valid) => {
        if (valid) {
          if (this.cheData?.value) {
            this.form.vehicleNumber = this.cheData.value
          }
          let data = {
            //手机号
            phone: this.form.phone,
            //司机密码
            //password: this.form.password,
            //身份证头像面
            idCardPerson: this.form.idCardPerson,
            //姓名
            name: this.form.name,
            //身份证
            idCardNumber: this.form.idCardNumber,
            //身份证有效期
            idCardLongEff: this.form.idCardLongEff,
            //身份证国徽面
            idCardBadge: this.form.idCardBadge,
            //身份证有效开始日期
            idCardPeriodStartTime: this.form.idCardPeriodStartTime,
            //身份证有效结束日期
            idCardPeriodEndTime: this.form.idCardPeriodEndTime,
            //驾驶证正本
            drivingPerson: this.form.drivingPerson,
            //驾驶证号
            drivingNumber: this.form.drivingNumber,
            //准驾车型
            vehicleType: this.form.vehicleType,
            licenseIssuingAuthority: this.form.licenseIssuingAuthority,
            //驾驶证副本
            drivingBadge: this.form.drivingBadge,
            //驾驶证有效开始日期
            drivingStartTime: this.form.drivingStartTime,
            //驾驶证有效结束日期
            drivingEndTime: this.form.drivingEndTime,
            //从业资格证
            cyCertificate: this.form.cyCertificate,
            //从业资格证2
            cyCertificateTwo: this.form.cyCertificateTwo,
            //从业资格证号
            qualifiedNumber: this.form.qualifiedNumber,
            //资格证有效开始日期
            qualifiedPeriodStartTime: this.form.qualifiedPeriodStartTime,
            //资格证有效结束日期
            qualifiedPeriodEndTime: this.form.qualifiedPeriodEndTime,
            //关联车辆
            vehicleNumber: this.form.vehicleNumber
          }
          saveDriver(data).then((res) => {
            if (res.code == 200) {
              this.phoneProblem = 0;
              this.$refs.form.resetFields();
              this.$message({
                message: '添加成功',
                type: 'success',
                duration: 3000
              });
              this.$router.push({ path: "/member/driver" })
            }
          });
        }
      });

    },
    //车辆数据
    handleSelect (item) {
      this.cheData = item;
    },
    //车辆数据
    querySearchAsync (queryString, cb) {
      queryVehicleNumber({ vehicleNumber: queryString }).then((res) => {
        if (res.code == 200) {
          var results = [];
          for (var i of res.data) {
            results.push({
              value: i
            })
          }
          cb(results);
        }
      });
    },
    //添加图片
    pictureIMG (data) {
      if (data?.cyCertificate) {
        this.form.cyCertificate = data.cyCertificate;
      } else if (data?.cyCertificateTwo) {
        this.form.cyCertificateTwo = data.cyCertificateTwo;
      }
    },
    //行驶证OCR返回数据
    drivingImg (data) {
      if (data?.idCardPerson) {
        this.form.idCardPerson = data.idCardPerson.fileUrl;//证件图片
        this.form.name = data.idCardPerson.name;//姓名
        this.form.idCardNumber = data.idCardPerson.idcard;//身份证号
        this.form.idCardAddress = data.idCardBadge.address//身份证地址
      } else if (data?.idCardBadge) {
        this.form.idCardBadge = data.idCardBadge.fileUrl;//证件图片
        var timelimit = data.idCardBadge.timelimit;//日期
        if (timelimit != "") {
          var i = timelimit.split('-');
        }
        this.form.idCardPeriodStartTime = i[0];//身份证有效开始日期
        this.form.idCardPeriodEndTime = i[1]; //身份证有效结束日期
        if (this.form.idCardPeriodStartTime != "") {
          var idCardPeriodStartTime = String(this.form.idCardPeriodStartTime)
          this.form.idCardPeriodStartTime = idCardPeriodStartTime.replace(/[.]/g, "-")
        }
        if (this.form.idCardPeriodEndTime != "") {
          var idCardPeriodEndTime = String(this.form.idCardPeriodEndTime)
          this.form.idCardPeriodEndTime = idCardPeriodEndTime.replace(/[.]/g, "-")
        }
      } else if (data?.drivingPerson) {
        this.form.drivingPerson = data.drivingPerson.fileUrl;//证件图片
        this.form.drivingNumber = data.drivingPerson.info_Positive.idcard;//驾驶证号
        this.form.vehicleType = data.drivingPerson.info_Positive.type;//准驾车型
        this.form.drivingStartTime = data.drivingPerson.info_Positive.begin_date;//驾驶证有效开始日期
        var a = String(this.form.drivingStartTime)
        var b = a.split("-")
        if (!b[1]) {
          this.form.drivingStartTime = a + '-01-01'
        } else if (b[1] == "") {
          this.form.drivingStartTime = a + '01-01'
        } else if (!b[2]) {
          this.form.drivingStartTime = a + '-01'
        } else if (b[2] == "") {
          this.form.drivingStartTime = a + '01'
        }
        if (data.drivingPerson.info_Positive.end_date == '长期') {
          this.form.drivingEndTime = "";
        } else {
          this.form.drivingEndTime = data.drivingPerson.info_Positive.end_date;//驾驶证有效结束日期
        }
      } else if (data?.drivingBadge) {
        this.form.drivingBadge = data.drivingBadge.fileUrl;//证件图片
      }
    },
    //删除图片
    pictureDelete (data) {
      this.form[data] = "";
    }
  }
}
</script>

<style scoped>
.el-divider--vertical {
  width: 5px;
  height: 1.5em;
  background-color: #1890ff;
}
</style>
