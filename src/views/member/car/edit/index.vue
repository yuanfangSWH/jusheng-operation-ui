...
export default {
  ...
  methods: {
    ...
    disappearIMG (key) {
      ...
      this.urls[key].url = "http://cc.yunsd56.com/files/2411532fe11d43bc8c6587821504e0f6?2411532fe11d43bc8c6587821504e0f6";
      this.form[this.urls[key].value] = "";
    },
    //切换图片
    getImg (key) {
    ...
    querySearchAsync (queryString, cb) {
      queryVehicleType({ VehicleTypeName: queryString }).then((res) => {
        if (res.code == 200) {
          var results = [];
          for (var i of res.data) {
            results.push({
              value: i.dictLabel,
              vehicleColorCode: i.dictValue
            })
          }
          cb(results);
        }
      ...
      this.$refs.form.validate((valid) => {
        if (valid) {
          if (this.cheData?.vehicleColorCode) {
            this.form.vehicleType = this.cheData.vehicleColorCode
          }
          let data = {
