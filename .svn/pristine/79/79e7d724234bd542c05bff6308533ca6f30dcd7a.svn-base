<template>
  <div class="app-container">
    <el-form ref="form" :model="form" :rules="rules" label-width="150px" inline>
      ...
      <el-form-item label="审核意见" v-show="form.status == 3">
        <el-input
          type="textarea"
    ...
    <div class="flex justify-center">
      <el-button
        ...
        >返回</el-button
      >
      <el-button type="primary" @click="updateShipper()">保存提交</el-button>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    queryShipperById () {
      queryShipperById({ shipperCode: this.form.shipperCode }).then((res) => {
        if (res.code == 200) {
          ...
          //图片组件初始化
          this.$refs.uploadIMG.originalData(this.form.businessLicense);
          this.$refs.uploadIMG1.originalData(this.form.idCardPerson);
          this.$refs.uploadIMG2.originalData(this.form.idCardBadge);
          this.$refs.uploadIMG3.originalData(this.form.companyPicture);
          this.$refs.uploadIMG4.originalData(this.form.frameContract);
        }
      });
    },
    wenrou () {
    ...
    //修改保存
    updateShipper () {
      if (this.form.status == 3 && this.form.notGoCause == "") {
        this.$message({
          message: '请填写审核意见',
          type: 'warning',
          duration: 2000
