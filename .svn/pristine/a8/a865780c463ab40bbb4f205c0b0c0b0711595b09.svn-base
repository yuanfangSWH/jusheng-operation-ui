    ...
    >
      ...
      <el-table-column prop="invoiceDetail" label="发票及明细">
        <template slot-scope="data">
          <a
            class="operate mr10"
            :href="data.row.invoiceDetail ? data.row.invoiceDetail : ''"
            target="_blank"
            style="color: #67c23a"
            >下载</a
          >
    ...
    >
      <el-form ref="form" :model="form" :rules="rules" label-width="150px">
        ...
        <el-form-item label="快递单号" prop="courierNumber">
          <el-input v-model="form.courierNumber" clearable></el-input>
        </el-form-item>
...
<script>
import { invoiceManagePage, uploadInvoice, deleteInvoice } from "@/api/finance/invoice";
...
export default {
  ...
  data () {
    return {
      ...
      tableData: [],
      rules: {
        invoiceTime: [{ required: true, message: "开票日期不能为空" }],
        invoiceDetail: [{ required: true, message: "发票及明细不能为空" }],
      },
      form: {
        applyCode: "", //开票申请号
        invoiceTime: "", //开票日期
  ...
  methods: {
    ...
    // 查询发票申请列表
    invoiceManagePage () {
      invoiceManagePage({ applyCode: this.form.applyCode }).then((res) => {
        if (res.code == 200) {
          if (res?.data) {
            if (res.data != '' && String(res.data) != 'null') {
