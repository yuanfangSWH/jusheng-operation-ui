  ...
  <div class="app-container">
    ...
    <el-table :data="tableData" style="width: 100%">
      ...
      <el-table-column prop="payState" label="支付状态" width="150px">
        <template slot-scope="data">
          <span v-if="data.row.payState == 0" style="color: #f56c6c"
            >未付款</span
          >
      ...
      <el-table-column prop="settleWeight" label="结算重量" width="170px">
        <template slot-scope="data">
      ...
      <el-table-column prop="unitPrice" label="单价" width="200px">
        <template slot-scope="data">
          {{ data.row.unitPrice
          }}<span v-if="data.row.unitPriceUnit == 0">元/吨</span>
          <span v-else-if="data.row.unitPriceUnit == 1">元/方</span>
          <span v-else-if="data.row.unitPriceUnit == 2">元/件</span>
          <span v-else-if="data.row.unitPriceUnit == 3">元/车</span>
        </template>
      </el-table-column>
    </el-table>
...
<script>
import { queryInvoiceById, queryCarriagesByApplyCode, invoiceAudit, exportWaybillRecord } from "@/api/business/invoice"
export default {
  data () {
    return {
      form: {
  ...
  methods: {
    ...
    //审核
    invoiceAudit () {
      if (this.auditStatus == 3 && this.auditRemark == "") {
        this.$message({
          message: "审核内容不能为空",
          type: "warning",
          duration: 2000,
      ...
      invoiceAudit({ applyCode: this.form.applyCode, auditRemark: this.auditRemark, auditStatus: this.auditStatus }).then((res) => {
        if (res.code == 200) {
          ...
          this.$router.push({ path: "/business/invoiceAudit" })
        }
      });
    },
    //详情
